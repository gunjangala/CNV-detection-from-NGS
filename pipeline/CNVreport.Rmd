...
merged <- as.data.frame(rbind(cbind(pindel$chromosome,as.numeric(pindel$position),as.numeric(pindel$SVLEN)),
                               cbind(lumpy$chromosome,as.numeric(lumpy$position),as.numeric(lumpy$SVLEN)),
                               cbind(svaba$chromosome,as.numeric(svaba$position),as.numeric(svaba$SVLEN))))
colnames(merged) <- c("chromosome","position","SVLEN")
...
votes <- NULL
votes <- as.data.frame(cbind(as.character(merged$chromosome),as.integer(merged$position),(as.integer(merged$position)+as.integer(merged$SVLEN)),merged$SVLEN),stringsAsFactors=FALSE)
colnames(votes) <- c("chromosome","start","end","length")
...
CNV.call.3 <- CNV.call.3 [order(CNV.call.3$chromosome,CNV.call.3$start),]
CNV.call.2 <- votes[votes$total.votes==2,]
CNV.call.2 <- CNV.call.2 [order(CNV.call.2$chromosome,CNV.call.2$start),]
CNV.call.1 <- votes[votes$total.votes==1,]
CNV.call.1 <- CNV.call.1 [order(CNV.call.1$chromosome,CNV.call.1$start),]
```
