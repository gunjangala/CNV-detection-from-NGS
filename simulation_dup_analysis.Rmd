...
```{r Voting,warning=FALSE,message=FALSE}
pindel.GR <- gRange.object(pindel.dup)
cnv.GR <- gRange.object(cnv.dup)
lumpy.GR <- gRange.object(lumpy.duph)
...
colnames(votes) <- c("chrom","start","end","size")
...
<<<<<<< HEAD
votes$PC <- (votes$pindel + votes$cnvnator)
votes$PL <- (votes$pindel + votes$lumpy)
votes$CL <- (votes$cnvnator + votes$lumpy)
votes$PCL <- (votes$pindel + votes$cnvnator + votes$lumpy)
=======


votes$call <- (votes$pindel + votes$cnvnator + votes$lumpy)
>>>>>>> b3c063df43680f8beec0c07f0d3344329755ec70
