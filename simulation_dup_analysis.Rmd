...
```{r reading in the files,echo=FALSE,eval=FALSE}
...
breakdancer <- read.csv("100_breakdancer.txt", sep=" ",comment.char="",header=FALSE,stringsAsFactors = FALSE)
breakdancer <-(separate(data = breakdancer , col =V1, into = c("sim", "chrom"), sep = "_bd.txt:"))
...
breakdancer.dup <- breakdancer[breakdancer$svtype=="DEL" & breakdancer$size>50 & breakdancer$supporting.reads>10 ,]
```
...
scores <- function (query,subject,algorithm,coverage) {
  ...
  analysis <-t(as.data.frame(as.matrix(c(type,coverage,algorithm,present,detected,TP,FP,FN,FDR,precision,sensitivity))))
...
#breakdancer.dup.analysis<-scores(breakdancer.dup,sim.dup,"breakdancer",depth)
...
write.csv(final, file = paste0(depth,".xls"))
```
...
#breakdancer.GR <- gRange.object(breakdancer.dup)
...
#breakdancer.vote <- sapply(countOverlaps(sim.GR,breakdancer.GR, type = "any"), function(x){if(x>0){return(2)}else{return(1)}})
...
#if (length(breakdancer.vote)>1){
#  votes$breakdancer <- breakdancer.vote
#} else {
#  votes$breakdancer <- 0
#}
